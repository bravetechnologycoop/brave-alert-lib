...
class BraveAlerter {
  ...
  async handleTwilioRequest(request, response) {
    try {
      ...
      const { nextAlertState, incidentCategoryKey, details, deviceName, returnMessage } = this.alertStateMachine.processStateTransitionWithMessage(
      ...
      // Store the results
      const changedAlertSession = new AlertSession(alertSession.sessionId, nextAlertState, incidentCategoryKey, details, deviceName)
      await this.alertSessionChangedCallback(changedAlertSession)
