...
class BraveAlerter {
  ...
  async handleTwilioRequest(request, response) {
    try {
      ...
      helpers.log(`!!! Request from: ${fromPhoneNumber}, Request to ${toPhoneNumber}`)
      helpers.log(
        `!!! Alert Session info - Alert state ${alertSession.alertState}, Message - ${message}, AlertSession incident categories keys: ${alertSession.validIncidentCategoryKeys}, AlertSession incident categories: ${alertSession.validIncidentCategories}, language: ${alertSession.language}`,
      )
      ...
      // Deconstruct stateTransition
      const { nextAlertState, incidentCategoryKey } = stateTransition
      let { returnMessageToRespondedByPhoneNumber, returnMessageToOtherResponderPhoneNumbers } = stateTransition

      helpers.log(`!!! Next alert - state: ${nextAlertState}, incidentCategoryKey - ${incidentCategoryKey}`)
      ...
      const { respondedByPhoneNumber, replacementReturnMessageToRespondedByPhoneNumber, replacementReturnMessageToOtherResponderPhoneNumbers } =
        await this.alertSessionChangedCallback(changedAlertSession)

      // replace return message to responded by phone number if it is defined (null is an accepted value)
      if (replacementReturnMessageToRespondedByPhoneNumber !== undefined) {
        returnMessageToRespondedByPhoneNumber = replacementReturnMessageToRespondedByPhoneNumber
      }

      // replace return message to other responder phone numbers if it is defined (null is an accepted value)
      if (replacementReturnMessageToOtherResponderPhoneNumbers !== undefined) {
        returnMessageToOtherResponderPhoneNumbers = replacementReturnMessageToOtherResponderPhoneNumbers
      }
