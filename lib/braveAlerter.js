...
class BraveAlerter {
  ...
  constructor(
    ...
    this.alertStateMachine = new AlertStateMachine(getReturnMessage)
  }
  ...
  async sendAlertSessionUpdate(sessionId, responderPushId, toPhoneNumber, fromPhoneNumber, textMessage, pushMessage) {
    ...
    try {
      if (responderPushId) {
        ...
        response = await twilioHelpers.sendTwilioMessage(toPhoneNumber, fromPhoneNumber, textMessage)
      }
    } catch (err) {
      helpers.logError(err)
    }
  ...
  async startAlertSession(alertInfo) {
    ...
    try {
      if (alertInfo.responderPushId) {
        ...
        response = await twilioHelpers.sendTwilioMessage(alertInfo.toPhoneNumber, alertInfo.fromPhoneNumber, alertInfo.message)
      }
    } catch (err) {
      // Error handling will be done below because response is undefined
      helpers.logError(err)
  ...
  async sendReminderMessageForSession(alertInfo) {
    ...
    try {
      ...
      if (alertInfo.responderPushId) {
        ...
        response = await twilioHelpers.sendTwilioMessage(alertInfo.toPhoneNumber, alertInfo.fromPhoneNumber, alertInfo.reminderMessage)
      }
    } catch (err) {
  ...
  async handleTwilioRequest(request, response) {
    try {
      ...
      const { nextAlertState, incidentCategoryKey, returnMessage } = this.alertStateMachine.processStateTransitionWithMessage(
      ...
      // Store the results
      const changedAlertSession = new AlertSession(alertSession.sessionId, nextAlertState, incidentCategoryKey)
      await this.alertSessionChangedCallback(changedAlertSession)

      // Send the response
      await twilioHelpers.sendTwilioResponse(response, returnMessage)
    } catch (err) {
      helpers.logError(err)
