...
class BraveAlerter {
  ...
  async startAlertSession(alertInfo) {
    ...
    if (alertInfo.responderPushId) {
      response = await OneSignal.sendOneSignalMessage(alertInfo.responderPushId, `${alertInfo.sessionId} START`, alertInfo.message, {
        sessionId: alertInfo.sessionId,
        alertType: alertInfo.alertType,
        deviceName: alertInfo.deviceName,
        fallbackTime:
          alertInfo.fallbackTimeoutMillis && alertInfo.fallbackTimeoutMillis > 0
            ? new Date(new Date().getTime() + alertInfo.fallbackTimeoutMillis)
            : new Date(),
      })

      console.log('TODO: Handle difference response failures')
