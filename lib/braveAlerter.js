    ...
    try {
      if (alertInfo.responderPushId) {
        ...
        response = await OneSignal.sendOneSignalMessage(
          ...
          `New ${helpers.getAlertTypeDisplayName(alertInfo.alertType, alertInfo.language, alertInfo.t)} Alert:\n${alertInfo.deviceName}`,
        )
    ...
    try {
      ...
      if (alertInfo.responderPushId) {
        ...
        response = await OneSignal.sendOneSignalMessage(
          ...
          `${helpers.getAlertTypeDisplayName(alertInfo.alertType, alertInfo.language, alertInfo.t)} Alert Reminder:\n${alertInfo.deviceName}`,
        )
      } else if (alertInfo.toPhoneNumbers && alertInfo.toPhoneNumbers.length > 0 && alertInfo.fromPhoneNumber) {
