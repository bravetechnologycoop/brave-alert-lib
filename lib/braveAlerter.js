...
class BraveAlerter {
  ...
  constructor(getAlertSession, getAlertSessionByPhoneNumber, getAlertSessionBySessionIdAndAlertApiKey, alertSessionChangedCallback, getLocationByAlertApiKey, getActiveAlertsByAlertApiKey, getHistoricAlertsByAlertApiKey, getNewNotificationsCountByAlertApiKey, asksIncidentDetails, getReturnMessage) {
  ...
  async handleAcknowledgeAlertSession(request, response) {
    ...
    try {
      ...
      const activeAlerts = this.getActiveAlertsByAlertApiKey(alertApiKey)
      if (activeAlerts) {
      ...
      helpers.logError(`Failed to acknowledge alert for session ${sessionId}: ${err.toString()}`)
      response.status(500).send()
    }
  }
  ...
  async handleRespondToAlertSession(request, response) {
    ...
    try {
      ...
      helpers.logError(`Failed to respond to alert for session ${sessionId}: ${err.toString()}`)
      response.status(500).send()
    }
  }
  ...
  async handleIncidentCategory(request, response) {
    ...
    try {
      ...
      helpers.logError(`Failed to record incident category ${incidentCategory} for session ${sessionId}: ${err.toString()}`)
      response.status(500).send()
    }
  }
}
